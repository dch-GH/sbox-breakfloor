...
partial class Player
{
	...
	private void OnFrame()
	{
		...
		var tr = Trace.Box( Vector3.One * 4, origin, dest ).Ignore( this ).Ignore( Gun ).Run();
		...
		if ( distance < pullbackCutoff )
		{
			...
			var backTr = Trace.Box( Vector3.One * 4, new Ray( origin, -dir ), 0.02f ).Ignore( this ).Ignore( Gun ).Run();
